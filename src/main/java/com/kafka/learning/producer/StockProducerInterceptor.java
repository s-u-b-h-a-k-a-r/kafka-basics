...
public class StockProducerInterceptor implements ProducerInterceptor<String, StockPrice> {
    ...
    @Override
    public ProducerRecord<String, StockPrice> onSend(final ProducerRecord<String, StockPrice> record) {
        onSendCount++;
        if (logger.isDebugEnabled()) {
            logger.debug(String.format("onSend topic=%s key=%s value=%s %d \n", record.topic(), record.key(), record.value().toString(), record.partition()));
        } else {
